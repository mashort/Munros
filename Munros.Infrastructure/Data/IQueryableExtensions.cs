...
namespace Munros.Infrastructure.Data
{
    public static class IQueryableExtensions
    {
        ...
        public static IQueryable<TEntity> ThenByCustom<TEntity>(this IQueryable<TEntity> items, string sortBy, string sortOrder)
        {
            var type = typeof(TEntity);
            var expression2 = Expression.Parameter(type, "t");
            var property = type.GetProperty(sortBy,
            ...
            var result = Expression.Call(
                typeof(Queryable),
                sortOrder == "desc" ? "ThenByDescending" : "ThenBy",
                new Type[] { type, property.PropertyType },
                items.Expression,
                Expression.Quote(lambda));
